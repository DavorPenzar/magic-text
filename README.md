...
The library provides simple interfaces and classes for tokenising existing text blocks and generating new ones upon the extracted tokens. The library is written in [*C# 8*](http://docs.microsoft.com/en-gb/dotnet/csharp/whats-new/csharp-8) compliant with [*.NET Standard 2.0*](http://github.com/dotnet/standard/blob/master/docs/versions/netstandard2.0.md)&emdash;but is actually targeted for newer versions&emdsah;with a few additional [*NuGet* packages](http://nuget.org/). However, the packages are included only to enable compatibility with [*.NET Standard 2.0*](http://github.com/dotnet/standard/blob/master/docs/versions/netstandard2.0.md) and to add special annotations/attributes to fields, properties, methods and method arguments, which could be omitted without the loss of the intended functionality provided by the library.
