...
namespace MagicText
{
    ...
    ///     <para>A derived class must minimally implement the <see cref="ShatterLine(String)" /> method to make a useful instance of the <see cref="LineShatteringTokeniser" />.</para>
    ...
    ///     <para>No thread safety mechanism is implemented nor assumed by the class. If the function for checking emptiness of tokens (<see cref="IsEmptyToken" />) should be thread-safe, lock the <see cref="LineShatteringTokeniser" /> instance during the complete <see cref="Shatter(TextReader, ShatteringOptions?)" /> and <see cref="ShatterAsync(TextReader, ShatteringOptions, Boolean, CancellationToken)" /> method calls to ensure consistent behaviour of the function over a single shattering operation.</para>
    /// </remarks>
    [CLSCompliant(true)]
    public abstract class LineShatteringTokeniser : Object, ITokeniser
    {
        ...
        protected LineShatteringTokeniser(Func<String?, Boolean> isEmptyToken) : base()
        {
        ...
        /// </remarks>
        public LineShatteringTokeniser() : this(String.IsNullOrEmpty)
        {
        }
