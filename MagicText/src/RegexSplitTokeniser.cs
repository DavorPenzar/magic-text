...
namespace MagicText
{
    ...
    public sealed class RegexSplitTokeniser : RegexTokeniser
    {
        ...
        ///     <para>If a match occurs exactly at the beginning or the end of the (line of) text to split, the splitting shall yield an empty <see cref="String" /> at the corresponding end of the input. The empty <see cref="String" />s (tokens) may be ignored by passing the adequate <see cref="ShatteringOptions" /> to the <see cref="LineShatteringTokeniser.Shatter(TextReader, ShatteringOptions)" /> and <see cref="LineShatteringTokeniser.ShatterAsync(TextReader, ShatteringOptions, Boolean, CancellationToken)" /> method calls, but this could unintentionally ignore some other tokens, possibly after transformation via the <see cref="TransformTokenFunc" /> function. Although the occurrence of empty strings is the case with splitting the input using any regular expression separator pattern in the described scenario(s), the remark is stated here because a grammatically correct text (i. e. organised in complete sentences) usually ends with a punctuation symbol, which is considered a separator by this pattern.</para>
        /// </remarks>
        /* language = regexp | jsregexp */
        [StringSyntax(StringSyntaxAttribute.Regex)]
        ...
        private readonly Func<String, String?>? _transformTokenFunc;
        ...
        private Func<String, String?>? TransformTokenFunc => _transformTokenFunc;
        ...
        /// <param name="transformTokenFunc">The optional token transformation function. If <c>null</c>, no transformation function is used.</param>
        ...
        ///     <code>
        ///         <see cref="RegexSplitTokeniser" />(pattern: <paramref name="regex" />.ToString(), options: <paramref name="alterOptions" /> ?? <paramref name="regex" />.Options, transformToken: <paramref name="transformTokenFunc" />)
        ///     </code>
        /// </remarks>
        public RegexSplitTokeniser(System.Text.RegularExpressions.Regex regex, Nullable<RegexOptions> alterOptions = default, Func<String, String?>? transformTokenFunc = null) : base(regex, alterOptions)
        {
            _transformTokenFunc = transformTokenFunc;
        }
        ...
        /// </remarks>
        public RegexSplitTokeniser(Boolean inclusiveSplitter, Func<String, String?>? transformTokenFunc = null) : this(inclusiveSplitter ? DefaultInclusiveRegex : DefaultExclusiveRegex, transformTokenFunc: transformTokenFunc)
        {
        }
        ...
        public RegexSplitTokeniser([StringSyntax(StringSyntaxAttribute.Regex)] String pattern, RegexOptions options = RegexOptions.None, Func<String, String?>? transformTokenFunc = null) : base(pattern, options)
        {
            _transformTokenFunc = transformTokenFunc;
        }
        ...
        ///     <para>After splitting the <c><paramref name="line" /></c> at regular expression separator pattern matches, the transformation of tokens is done using the <see cref="TransformTokenFunc" /> function if it is set.</para>
        ...
        protected override IEnumerable<String?> ShatterLine(String line)
        {
            ...
            if (!(TransformTokenFunc is null))
            {
                tokens = tokens.Select(TransformTokenFunc!);
            }
