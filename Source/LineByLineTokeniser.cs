...
namespace MagicText
{
    ...
    ///     <para>Assuming the default behaviour of the <see cref="TextReader" /> used concerning its <see cref="TextReader.ReadLine()" /> and <see cref="TextReader.ReadLineAsync()" /> methods (such as <see cref="StringReader" />'s and <see cref="StreamReader" />'s behaviour), shattering methods read and process text <em>line-by-line</em> with all CR, LF and CRLF line breaks treated the same. These line breaks and the end of the input are considered line ends when shattering text, and are therefore substituted by a <see cref="ShatteringOptions.LineEndToken" /> if <see cref="ShatteringOptions.IgnoreLineEnds" /> is <c>false</c>. This behaviour may not be overridden by a derived class.</para>
    ...
    public abstract class LineByLineTokeniser : Object, ITokeniser
    {
        ...
        ///     <para>It is guaranteed that, when called from the <see cref="LineByLineTokeniser" />'s non-overridable methods, the <c><paramref name="line" /></c> shall be a non-<c>null</c> <see cref="String" /> that does not contain a line end (CR, LF or CRLF). Nonetheless, when calling from a derived class, its programmer may call the method however they wish.</para>
        /// </remarks>
