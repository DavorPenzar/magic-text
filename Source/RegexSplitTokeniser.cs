...
namespace MagicText
{
    /// <summary>Implements a <see cref="LineShatteringTokeniser" /> which shatters lines of text by splitting it at specific regular expression pattern matches.</summary>
    /// <remarks>
    ...
    ///     <para>If a default regular expression break pattern (<see cref="DefaultInclusivePattern" /> or <see cref="DefaultExclusivePattern" />) should be used without special <see cref="RegexOptions" />, a better performance is achieved when using the default <see cref="RegexSplitTokeniser()" /> constructor or the <see cref="RegexSplitTokeniser(Boolean, Func{String, String})" /> constructor, in which case a pre-built <see cref="System.Text.RegularExpressions.Regex" /> object is used (constructed with <see cref="System.Text.RegularExpressions.Regex.Options" /> set to <see cref="RegexTokeniser.DefaultOptions" />), instead of the <see cref="RegexSplitTokeniser(String, Boolean, RegexOptions, Func{String, String})" /> constructor.</para>
    ///     <para>Empty tokens (which are ignored if <see cref="ShatteringOptions.IgnoreEmptyTokens" /> is <c>true</c>) are considered those tokens that yield <c>true</c> when checked via the <see cref="String.IsNullOrEmpty(String)" /> method after the potential transformation via the <see cref="Transform" /> function if it is set. This behaviour cannot be overridden by a derived class.</para>
    ///     <para>No thread safety mechanism is implemented nor assumed by the class. If the transformation function (<see cref="Transform" />) should be thread-safe, lock the <see cref="RegexSplitTokeniser" /> instance during complete <see cref="ShatterLine(String)" />, <see cref="LineShatteringTokeniser.Shatter(TextReader, ShatteringOptions)" /> and <see cref="LineShatteringTokeniser.ShatterAsync(TextReader, ShatteringOptions, Boolean, CancellationToken)" /> method calls to ensure consistent behaviour of the function over a single shattering operation.</para>
    /// </remarks>
    [CLSCompliant(true)]
    public class RegexSplitTokeniser : RegexTokeniser
    {
        ...
        ///     <para>If a match occurs exactly at the beginning or the end of the (line of) text to split, the splitting shall yield an empty <see cref="String" /> at the corresponding end of the input. The empty <see cref="String" />s (tokens) may be ignored by passing the adequate <see cref="ShatteringOptions" /> to the <see cref="LineShatteringTokeniser.Shatter(TextReader, ShatteringOptions)" /> and <see cref="LineShatteringTokeniser.ShatterAsync(TextReader, ShatteringOptions, Boolean, CancellationToken)" /> method calls, but this could unintentionally ignore some other tokens, possibly after transformation via the <see cref="Transform" /> delegate. Although the occurrence of empty strings is the case with splitting the input using any regular expression separator pattern in the described scenario(s), the remark is stated here because a grammatically correct text (i. e. organised in complete sentences) usually ends with a punctuation symbol, which is considered a separator by this pattern.</para>
        /// </remarks>
        /* language = regexp | jsregexp */
        [RegexPattern]
        ...
        /// </remarks>
        protected static System.Text.RegularExpressions.Regex DefaultInclusiveRegex => _defaultInclusiveRegex;
        ...
        /// </remarks>
        protected static System.Text.RegularExpressions.Regex DefaultExclusiveRegex => _defaultExclusiveRegex;
        ...
        ///     <code>
        ///         <see cref="RegexSplitTokeniser" />(pattern: <paramref name="regex" />.ToString(), options: <paramref name="alterOptions" /> ?? <paramref name="regex" />.Options, transform: <paramref name="transform" />)
        ///     </code>
        /// </remarks>
        public RegexSplitTokeniser(System.Text.RegularExpressions.Regex regex, Nullable<RegexOptions> alterOptions = default, Func<String?, String?>? transform = null) : base(regex, alterOptions)
        {
        ...
        /// </remarks>
        public RegexSplitTokeniser(Boolean inclusiveSplitter, Func<String?, String?>? transform = null) : this(inclusiveSplitter ? DefaultInclusiveRegex : DefaultExclusiveRegex, transform: transform)
        {
        }
        ...
        /// <param name="escape">If <c>true</c>, the <c><paramref name="pattern" /></c> is escaped using the <see cref="System.Text.RegularExpressions.Regex.Escape(String)" /> method to construct the actual regular expression separator pattern.</param>
        ...
        public RegexSplitTokeniser([RegexPattern] String pattern, Boolean escape = false, RegexOptions options = RegexOptions.None, Func<String?, String?>? transform = null) : base(pattern, escape, options)
        {
            _transform = transform;
        }
        ...
        ///     <para>After splitting the <c><paramref name="line" /></c> at regular expression separator pattern matches, the transformation of tokens is done using the <see cref="Transform" /> function if it is set.</para>
